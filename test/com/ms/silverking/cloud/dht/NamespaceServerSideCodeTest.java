...
public class NamespaceServerSideCodeTest {
  ...
  private static final NamespaceServerSideCode defaultCodeCopy = new NamespaceServerSideCode(urlCopy, putTriggerCopy,
      retrieveTriggerCopy);
  private static final NamespaceServerSideCode defaultCodeAlmostCopy = new NamespaceServerSideCode(urlCopy,
      putTriggerCopy, retrieveTriggerDiff);
  private static final NamespaceServerSideCode defaultCodeDiff = new NamespaceServerSideCode(urlDiff, putTriggerDiff,
      retrieveTriggerDiff);
  ...
  public void testGetters() {
    String[][] testCases = { { urlCopy, getUrl(defaultCode) }, { putTriggerCopy, getPutTrigger(defaultCode) },
        { retrieveTriggerCopy, getRetrieveTrigger(defaultCode) },

        { urlDiff, getUrl(defaultCodeDiff) }, { putTriggerDiff, getPutTrigger(defaultCodeDiff) },
        { retrieveTriggerDiff, getRetrieveTrigger(defaultCodeDiff) },

        { null, getUrl(defaultCodeNull1) }, { null, getPutTrigger(defaultCodeNull1) }, { null, getRetrieveTrigger(
        defaultCodeNull1) }, };
  ...
  public void testHashCode() {
    ...
    checkHashCodeEquals(defaultCode,
        defaultCodeNull1);    // TODO:bph: strictly we would want checkHashNotEquals(), but this is fine for now
    // since hashCode doesn't need to be unique, and both are corner cases
    checkHashCodeEquals(defaultCodeNull1, defaultCodeNull2);
  }

  @Test
  public void testEqualsObject() {
    NamespaceServerSideCode[][] testCases = { { defaultCode, defaultCode, defaultCodeDiff },
        { defaultCodeCopy, defaultCode, defaultCodeDiff },
        { defaultCodeAlmostCopy, defaultCodeAlmostCopy, defaultCode },
        { defaultCodeDiff, defaultCodeDiff, defaultCode }, { defaultCodeNull1, defaultCodeNull2, defaultCode }, };
    test_FirstEqualsSecond_FirstNotEqualsThird(testCases);

    test_NotEquals(new Object[][] { { defaultCode, NamespaceOptions.templateOptions }, });
  }
